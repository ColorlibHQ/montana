...
function montana_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_montana_'.$id, true );
    
    return $value;
}
// Blog Date Permalink
function montana_blog_date_permalink(){
...
// Blog Excerpt Length
if ( ! function_exists( 'montana_excerpt_length' ) ) {
	function montana_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'montana_posted_comments' ) ) :
    function montana_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','montana');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','montana');
            } else {
                $comments = esc_html__( '1 Comment','montana' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'montana' );
        }
...
function montana_iframe_match(){   
    $audio_content = montana_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function montana_embedded_media( $type = array() ){
...
// WP post link pages
function montana_link_pages(){
    
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'montana' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'montana' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// Blog social sharing buttons
function montana_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
...
if( ! function_exists('montana_blog_single_post_navigation') ) {
	function montana_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'montana_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'montana' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'montana' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'montana_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Montana Comment Template Callback
function montana_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'montana' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'montana'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'montana' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'montana' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'montana_replace_reply_link_class');
function montana_replace_reply_link_class( $class ) {
...
// theme logo
function montana_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'montana_site_logo_112x92' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( montana_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
...
function montana_pull_right( $id = '', $condation ){
...
// image alt tag
function montana_image_alt( $url = '' ){
...
function montana_get_textareahtml_output( $content ) {
...
function montana_get_slider_shortcode_options( $field ) {
...
// html Style tag
function montana_inline_bg_img( $bgUrl ){
    $bg = '';
...
function montana_sidebar_opt(){

    $sidebarOpt = montana_opt( 'montana-blog-sidebar-settings' );
    $sidebar = '1';
...
function montana_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'montana' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'montana' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function montana_themify_icon(){
    return[
        'flaticon-layers'     => __('Layer Icon', 'montana'),
        'flaticon-design'     => __('Design Icon', 'montana'),
        'flaticon-speaker'    => __('Speaker Icon', 'montana'),
    ];
}

// CTA section
function montana_get_cta_area() {
	$cta_section_title      = !empty( montana_opt( 'montana_cta_section_title') ) ? montana_opt( 'montana_cta_section_title') : '';
	$cta_section_text       = !empty( montana_opt( 'montana_cta_section_text') ) ? montana_opt( 'montana_cta_section_text') : '';
	$cta_section_btn_label  = !empty( montana_opt( 'montana_cta_section_btn_label') ) ? montana_opt( 'montana_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( montana_opt( 'montana_cta_section_btn_url') ) ? montana_opt( 'montana_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function montana_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = montana_opt('montana_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = MONTANA_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = MONTANA_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = MONTANA_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
