...
function montana_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'montana' ),
        'next_string'     => esc_html__( '&raquo;', 'montana' ),
        'before_output'   => '<div class="montana-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'montana_pagination_defaults', $defaults )
    );
