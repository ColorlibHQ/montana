...
function montana_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'montana' ) . '</span></li>';
...
// Blog categories
function montana_featured_post_cat(){
...
// social media
if ( ! function_exists( 'montana_social' ) ) {
	function montana_social( $args = array() ) {
		...
		$url = montana_opt( 'montana_social_url' );
...
// header cart count
function montana_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( MONTANA_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'montana' ); ?>">
...
function montana_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'montana_contact7_form_content', 10, 2 );